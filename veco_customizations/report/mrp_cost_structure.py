...
class MrpCostStructure(models.AbstractModel):
    ...
    def get_lines(self, productions):
        ...
        for product in productions.mapped('product_id'):
            ...
            total_cost = 0.0
            # variables to calc cost share (i.e. between products/byproducts)
            # since MOs can have varying distributions
            ...
            if Workorders:
                ...
                for mo_id, dummy_wo_id, op_id, wo_name, user, duration, cost_hour, currency_rate in self.env.cr.fetchall():
                    if not cost_hour:
                        cost_hour = self.env['mrp.workorder'].browse(
                            dummy_wo_id).workcenter_id.costs_hour
