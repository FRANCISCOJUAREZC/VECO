    ...
    def _l10n_mx_edi_get_payment_cfdi_values(self, move):
        ...
        for invoice_vals in result.get('invoice_vals_list', []):
            ...
            payments = payments.filtered(
                lambda p: p.journal_id.type in ['bank', 'cash'])
            payments_count = len(payments)
            reconciled_lines = invoice_vals['invoice'].line_ids.filtered(
            ...
            for reconciled_aml in reconciled_amls:
                if reconciled_aml.payment_id and reconciled_aml.payment_id in payments:
                    continue
                if reconciled_aml.payment_id and reconciled_aml.journal_id.type not in ['bank', 'cash']:
                    continue
                payments_count += 1
