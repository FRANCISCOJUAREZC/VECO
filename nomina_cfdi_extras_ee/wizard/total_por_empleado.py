    ...
    def print_total_por_empleado_report(self):
        ...
        if self.hr_payslip_run_ids:
            ...
            for t in ['Total Efectivo', 'Total Especie']:
                ...
                col_nm += 1
            row = 1
            hr_payslips = self.hr_payslip_run_ids.mapped('slip_ids')
            ...
            for employee_wise,datas in employee.items():
                ...
                for rule_name in all_col_list_seq:
                    ...
                    col+= 1 
                worksheet.write(row,col, slip.get_total_code_value('001')[0], text_right)
                col += 1
                worksheet.write(row,col, slip.get_total_code_value('002')[0], text_right)
                row+=1
                col = 0
        code_col = 4
        count=2
